...
def main():
    ...
    w('```')
    w()
    w('### 成本核算：TypeSafe 控制台真实账单')
    w()
    w('上表的 $%.5f 是按 token 单价折算的**估算值**。下面是同期 TypeSafe 控制台的'
      % s['cost_usd'])
    w('**真实账单**截图，可作交叉验证：')
    w()
    w('![TypeSafe 控制台 Usage 页面](docs/images/typesafe-usage-2026-09-22.png)')
    w()
    w('| 账单口径（最近 7 天，含全部调试调用） | 数值 |')
    w('|---|---|')
    w('| 总消费 Spend | **$0.0411** |')
    w('| 总 Tokens | **1,063,366** |')
    w('| 总 Requests | **1,873** |')
    w('| 计价方式 | `$0.042 / MTok` input，output 免费 |')
    w('| 单次请求均价 | **约 $0.000022** |')
    w()
    w('> 1,873 次请求 = 本次全量评测 1,497 次 + 前期的接口连通性、稳定性、低置信专项')
    w('> 与选项打乱等测试约 376 次。其中全量评测实耗 **$0.0317**（集中在 Sep 22 那一小时），')
    w('> 与按 token 折算的估算值一致。')
    w()
